  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = body?.detail?.title || body?.detail?.[0]?.msg || `Error ${res.status}`
        throw new Error(msg)
      }
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: '#EEF2FF' }}>
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl p-8 shadow-xl space-y-5"
          ...
          <h2 className="text-2xl font-black" style={{ color: '#0D1F3C' }}>{title}</h2>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wide mb-1.5" style={{ color: '#2952A3' }}>
              Email
            </label>
            <input
              type="email"
          ...
          <div>
            ...
            <input
              ...
              style={{ borderColor: '#C4D4F5', backgroundColor: '#F0F5FF', color: '#0D1F3C' }}
            />
            {isSignUp && <p className="text-[11px] mt-1" style={{ color: '#4A7AC7' }}>Minimum 8 characters.</p>}
          </div>
          ...
          <div className="flex items-center gap-3 my-1">
            <div className="flex-1 h-px" style={{ backgroundColor: '#C4D4F5' }} />
            <span className="text-[10px] uppercase tracking-wide" style={{ color: '#4A7AC7' }}>or</span>
            <div className="flex-1 h-px" style={{ backgroundColor: '#C4D4F5' }} />
          </div>

          <a
            href={`${API}/api/oauth/google/login`}
            className="w-full inline-flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold transition-all"
            style={{ backgroundColor: '#fff', color: '#0D1F3C', border: '1px solid #C4D4F5' }}
          >
          ...
          {!isSignUp && (
            <div className="text-center text-xs">
              <Link href="/forgot-password" className="underline" style={{ color: '#1B3A6B' }}>Forgot password?</Link>
            </div>
          )}

          <div className="text-center text-xs" style={{ color: '#2952A3' }}>
            {isSignUp ? (
              <>Already have an account?{' '}
                <Link href="/sign-in" className="font-bold underline" style={{ color: '#1B3A6B' }}>Sign in</Link>
              </>
            ) : (
              <>New to PropertyState?{' '}
                <Link href="/sign-up" className="font-bold underline" style={{ color: '#1B3A6B' }}>Create an account</Link>
              </>
            )}
          </div>
        </form>

        <p className="text-center text-xs mt-5" style={{ color: '#4A7AC7' }}>
